...
const options: Record<string, string[]> = {
  competitionStatus: ["upcoming", "active", "finished"], matchStatus: ["scheduled", "live", "finished", "postponed"], tier: ["gold", "silver", "bronze"], platform: ["youtube", "instagram", "tiktok"], event_type: ["goal", "assist", "yellow_card", "red_card", "substitution", "foul", "presidential_penalty", "other"],
};
...
function relationOptions(field: string, resource: Resource, form: Row, l: Lookups) {
  ...
  if (field === "player_id" || field === "related_player_id") { let ps = l.players; if (resource === "lineups" && form.team_id) ps = ps.filter((p) => p.team_id === form.team_id); if (["events", "mvp"].includes(resource) && form.match_id) { const m = l.matches.find((x) => x.id === form.match_id); const ids = m ? [m.home_team_id, m.away_team_id] : []; ps = ps.filter((p) => ids.includes(p.team_id)); } if (resource === "events" && field === "player_id" && form.event_type === "presidential_penalty") ps = ps.filter((p) => p.position?.trim().toUpperCase() === "STAFF"); return ps.map((p) => ({ id: p.id, label: `#${p.shirt_number ?? "—"} ${p.first_name} ${p.last_name}` })); }
  return [];
}
...
function validate(resource: Resource, payload: Row, l: Lookups) {
  ...
  if (resource === "events" && payload.event_type === "presidential_penalty") { const player = l.players.find((x) => x.id === payload.player_id); if (!player || player.position?.trim().toUpperCase() !== "STAFF") throw new Error("Per un rigore presidenziale devi selezionare un giocatore con ruolo STAFF."); }
...
function Crud({ resource, profile }: { resource: Resource; profile: AdminProfile }) {
  ...
  const setField = (f: string, v: unknown) => setForm((p) => ({ ...p, [f]: v, ...(f === "competition_id" ? { home_team_id: "", away_team_id: "" } : {}), ...(f === "match_id" ? { team_id: "", player_id: "", related_player_id: "" } : {}), ...(f === "team_id" && resource === "lineups" ? { player_id: "", related_player_id: "" } : {}) }));
